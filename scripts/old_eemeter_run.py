...
def generate_energy_data(start_date,
    ...
    ww = ISDWeatherSource(station_code)  # or another 6-digit USAF station
...
if __name__ == '__main__':
    ...
    dt = generate_date_range(usage_start,usage_end_date)

    normalized = False
    use_cz = True
    ww =  ISDWeatherSource(station_code)
    #ww = TMY3WeatherSource(station_code)
    ...
    print("Number NA in reporting", reporting_data.isnull().sum())

    baseline_model = build_model(baseline_data)
    print(reporting_data.tempF)
